...
// 图表对象
// let rootCauseChart, improvementChart;
// let statusChart, rootCauseChart, improvementChart;
// 新增：是否需要渲染图表的标志
...
// 数据表格主体，用于显示加载和筛选后的数据
const dataTableBody = document.getElementById('dataTableBody');

// 筛选按钮，点击后根据用户设置的筛选条件对数据进行过滤
...
// /**
//  * 渲染图表，包括状态分布、根本原因分布和改善措施实施情况
//  */
// function renderCharts() {
//     // 新增：只有当shouldRenderCharts为true时才渲染图表
//     if (!shouldRenderCharts) {
//         return;
//     }
//
//     // 筛选出BRDTEST站点的ADB_WAIT_BOOT_COMPLETE问题
//     const filteredBRDData = filteredData.filter(item =>
//         item['站点'] === 'BRDTEST' &&
//         item['问题描述'] === 'ADB_WAIT_BOOT_COMPLETE'
//     );
//
//     if (filteredBRDData.length === 0) {
//         // 如果没有数据，清空图表
...
//
//     // 按日期排序
//     filteredBRDData.sort((a, b) => new Date(a['日期']) - new Date(b['日期']));
//
//     // 准备状态数据
//     const statusData = getStatusDistribution(filteredBRDData);
//
//     // 准备根本原因数据
//     const rootCauseData = getRootCauseDistribution(filteredBRDData);
//
//     // 准备改善措施数据
//     const improvementData = getImprovementDistribution(filteredBRDData);
//
//     // 准备时间序列数据
//     const timeSeriesData = getTimeSeriesData(filteredBRDData);
//
//     // 渲染状态分布图表
//     renderStatusChart(statusData);
//
//     // 渲染根本原因分布图表
//     renderRootCauseChart(rootCauseData);
//
//     // 渲染改善措施实施情况图表
//     renderImprovementChart(improvementData);
// }

/**
 * 获取状态分布数据
 * @param {Array} data 筛选后的数据
 * @returns {Object} 状态分布数据
 */
 ...
 * 获取根本原因分布数据
 * @param {Array} data 筛选后的数据
 * @returns {Object} 根本原因分布数据
 */
function getRootCauseDistribution(data) {
    const rootCauseMap = {};
    data.forEach(item => {
        const rootCause = item['根本原因'] || '未知';
        rootCauseMap[rootCause] = (rootCauseMap[rootCause] || 0) + 1;
    });

    // 只显示前5个根本原因，其他的合并为"其他"
    const sortedRootCauses = Object.entries(rootCauseMap)
        .sort((a, b) => b[1] - a[1]);

    const topRootCauses = sortedRootCauses.slice(0, 5);
    const otherCount = sortedRootCauses.slice(5).reduce((sum, [, count]) => sum + count, 0);

    if (otherCount > 0) {
        topRootCauses.push(['其他', otherCount]);
    }

    return {
        labels: topRootCauses.map(([label]) => label),
        values: topRootCauses.map(([, count]) => count)
    };
}
 ...
 * 获取改善措施分布数据
 * @param {Array} data 筛选后的数据
 * @returns {Object} 改善措施分布数据
 */
function getImprovementDistribution(data) {
    const improvementMap = {};
    data.forEach(item => {
        const improvement = item['改善措施'] || '未知';
        improvementMap[improvement] = (improvementMap[improvement] || 0) + 1;
    });

    // 只显示前5个改善措施，其他的合并为"其他"
    const sortedImprovements = Object.entries(improvementMap)
        .sort((a, b) => b[1] - a[1]);

    const topImprovements = sortedImprovements.slice(0, 5);
    const otherCount = sortedImprovements.slice(5).reduce((sum, [, count]) => sum + count, 0);

    if (otherCount > 0) {
        topImprovements.push(['其他', otherCount]);
    }

    return {
        labels: topImprovements.map(([label]) => label),
        values: topImprovements.map(([, count]) => count)
    };
}
...
function getTimeSeriesData(data) {
    const dateMap = {};
    data.forEach(item => {
        const date = new Date(item['日期']).toISOString().split('T')[0];
...
function renderStatusChart(data) {
    ...
    // 为每个状态分配颜色
    const backgroundColor = data.labels.map(label => statusColors[label] || '#9CA3AF');
    ...
    rootCauseChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                data: data.values,
                backgroundColor: [
                    '#165DFF', '#36D399', '#FFAA33', '#FF5252', '#9CA3AF', '#6366F1'
                ],
                borderWidth: 0,
                hoverOffset: 4
            }]
...
function renderCharts() {
    ...
    // 只有当isRendering或者shouldRenderCharts为true时才渲染图表
    ...
    try {
        ...
        // 渲染日期与问题关系图表
        renderDateIssueChart(filteredChartData);
 ...
 * 渲染日期与问题关系图表
 */
function renderDateIssueChart(data) {
    // 确保DOM元素存在
    const canvas = document.getElementById('dateIssueChart');
    if (!canvas) {
        console.error('dateIssueChart canvas element not found');
        return;
    }
            ...
            options: {
                ...
                plugins: {
                    tooltip: {
                        ...
                        callbacks: {
                            label: function(context) {
                                const rootCause = datasets[context.datasetIndex].label;
                                const value = context.raw;
