...
let allData = [];
let filteredData = [];
const jsonFilePath = '../docs/TITANS项目_SOVP2_Report__0615_FE_NTF.json';
let currentPage = 1;
...
function addFilterCondition() {
    ...
    newGroup.innerHTML = `
        ...
        <div>
            ...
            <div class="relative">
                ...
                <div class="filter-suggestions absolute left-0 right-0 mt-1 bg-white rounded-md shadow-lg z-10 hidden"></div>
            </div>
        </div>
    ...
    // 添加输入事件监听
    const input = newGroup.querySelector('.filter-value');
    input.addEventListener('input', handleFilterValueInput);

    // 添加失去焦点事件
    input.addEventListener('blur', () => {
        setTimeout(() => {
            newGroup.querySelector('.filter-suggestions').classList.add('hidden');
        }, 200);
    });
}
...
function handleFilterValueInput(event) {
    ...
    const suggestionsContainer = group.querySelector('.filter-suggestions');

    // 清空现有建议
    suggestionsContainer.innerHTML = '';

    if (value.length < 2) { // 至少输入2个字符才显示建议
        suggestionsContainer.classList.add('hidden');
        return;
    }

    // 过滤匹配的值
    const matchingValues = allData
       .map(item => item[field])
       .filter((val, index, self) => val !== undefined && val !== null && self.indexOf(val) === index && String(val).toLowerCase().includes(value.toLowerCase()));

    if (matchingValues.length > 0) {
        // 显示匹配的值
        matchingValues.forEach(val => {
            const suggestion = document.createElement('div');
            suggestion.className = 'px-3 py-2 hover:bg-gray-100 cursor-pointer';
            suggestion.textContent = val;

            // 高亮匹配部分
            const regex = new RegExp(`(${value})`, 'gi');
            suggestion.innerHTML = suggestion.textContent.replace(regex, '<span class="font-semibold text-primary">$1</span>');
